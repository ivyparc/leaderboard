create table if not exists public.app_leaderboard_scores (
  ...
  primary key (app_id, player_id, period)
);

create unique index if not exists app_leaderboard_unique_name_idx
on public.app_leaderboard_scores (app_id, period, lower(name));

create index if not exists app_leaderboard_ranking_idx
